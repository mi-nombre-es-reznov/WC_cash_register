...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button24->Size = System::Drawing::Size(70, 19);
			...
			// button25
			// 
			this->button25->Location = System::Drawing::Point(77, -1);
			this->button25->Margin = System::Windows::Forms::Padding(2);
			this->button25->Name = L"button25";
			this->button25->Size = System::Drawing::Size(70, 19);
...
bool Tax_pressed, Discount_pressed, IP_pressed, clear_db = false;
...
private: void mySubscriber2a(System::Object^ sender, System::EventArgs^ e, String^ ip, String^ un, String^ pw, int port_num)
{
	...
	// Connections to database - Test Connection with user-entered data
	String ^ constring = L"datasource = '" + IP_addr + "'; port = '"+ port +"'; username = '" + User + "'; password = '" + PW + "'";
	MySqlConnection ^ conDatabase = gcnew MySqlConnection(constring);
	MySqlCommand ^ cmdDataBase = gcnew MySqlCommand("DELETE FROM `women_center_db`.`wc_data`;", conDatabase);
...
private: System::Void button25_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	String ^ constring = L"datasource = '" + IP_addr + "'; port = '" + port + "'; username = '" + User + "'; password = '" + PW + "'";
	MySqlConnection ^ conDatabase = gcnew MySqlConnection(constring);
	MySqlCommand ^ cmdDataBase = gcnew MySqlCommand("TRUNCATE TABLE `women_center_db`.`wc_data`", conDatabase);
	MySqlCommand ^ cmdDataBase_auto_increment = gcnew MySqlCommand("ALTER table `women_center_db`.`wc_data` AUTO_INCREMENT = 1", conDatabase);
	MySqlDataReader ^ myReader;
	...
	try
	{
		...
		conDatabase->Close();
		conDatabase->Open();
		myReader = cmdDataBase_auto_increment->ExecuteReader();
	}
	catch (Exception ^ ex)
	{
		MessageBox::Show(ex->Message);
